...
# Five different model architectures were experimented with during this project
# 1) BiLSTM MODEL
...
# 2) BiLSTM MODEL with flatten 

# def BiLSTM(input_shape, num_classes):
#     inputs = Input(shape=input_shape)  # shape: (1500, 128)
#     x = Bidirectional(LSTM(128, return_sequences=True))(inputs)  # -> (1500, 256)
#     x = BatchNormalization()(x)
#     x = Bidirectional(LSTM(128, return_sequences=True))(x)       # -> (1500, 256)
#     # Reduce the feature size before flattening
#     x = TimeDistributed(Dense(32, activation='relu'))(x)         # -> (1500, 32)
#     x = Flatten()(x)                                             # -> (1500 * 32 = 48000)
#     x = BatchNormalization()(x)
#     x = Dense(32, activation='relu')(x)                          
#     x = Dropout(0.3)(x)
#     outputs = Dense(num_classes, activation='softmax')(x)
...
# 3) BiLSTM MODEL with global average pooling
def BiLSTM(input_shape, num_classes):
    inputs = Input(shape=input_shape)
    x = Bidirectional(LSTM(128, return_sequences=True))(inputs)
    x = BatchNormalization()(x)
...
# 4) BiLSTM MODEL (Average_pooling)
# def BiLSTM(input_shape, num_classes):
#     inputs = Input(shape=input_shape)
#     x = Bidirectional(LSTM(128, return_sequences=True))(inputs)
#     x = BatchNormalization()(x)
...
# 5) BiLSTM MODEL (Mean across feature)
# def BiLSTM(input_shape, num_classes):
#     inputs = Input(shape=input_shape)
#     x = Bidirectional(LSTM(128, return_sequences=True))(inputs)
#     x = BatchNormalization()(x)
